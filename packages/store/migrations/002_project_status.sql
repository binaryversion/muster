...
ALTER TABLE projects
  ADD COLUMN github_project_id      text,   -- ProjectV2 node id
  ADD COLUMN github_status_field_id text,   -- ProjectV2SingleSelectField node id for "Status"
  -- Option name -> option id, exactly as the board defines them.
  ADD COLUMN github_status_options  jsonb NOT NULL DEFAULT '{}',
  ...
  ADD COLUMN github_status_map      jsonb,
  ADD COLUMN github_fields_synced_at timestamptz;

-- ProjectV2Item node id for the task's issue, resolved lazily by the sync loop
-- and by the projects_v2_item webhook.
ALTER TABLE tasks ADD COLUMN mirrored_at timestamptz;
...
CREATE INDEX tasks_unmirrored_idx ON tasks (project_id)

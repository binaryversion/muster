 ...
 *
 * SOURCE OF TRUTH. Copied into each package's src/crypto.ts by scripts/sync-shared.sh;
 * do not edit the copies. They exist because each package installs separately
 * and each Dockerfile's build context is its own directory, so a file:../shared
 * dependency would resolve at dev time and fail in the image.
 *
 * Both services have to agree on how a lead token is digested or leads stop
 * authenticating, so the copies are regenerated by a prebuild step and CI fails
